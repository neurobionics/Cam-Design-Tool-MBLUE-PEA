...
for i = 1:tasks
    if strcmp(tasklist{i}, 'squat25lb')
        tasktitlelist{i} = 'Squat';
    elseif strcmp(tasklist{i}, 'normalwalk1x2')
        tasktitlelist{i} = 'Level';
    elseif strcmp(tasklist{i}, 'rampascent')
        tasktitlelist{i} = 'Ramp Up';
    elseif strcmp(tasklist{i}, 'rampdescent')
        tasktitlelist{i} = 'Ramp Down';
    end
end
...
if figTF
    ...
    title = 'Stiffness, Torque, Power Plots (Tasks by Row)';
    taskstitle = "";
    ...
    for i = 1:tasks
        ...
        taskstitle = taskstitle + tasktitlelist{i};
        if ~(i==tasks)
            taskstitle = taskstitle + ', ';
        end      
    end
    sgtitle({title,taskstitle})
    ...
    set(hfig,'Units','centimeters','Position',[3 3 23.8464   21.5533])
    pos = get(hfig,'Position');
    set(hfig,'PaperPositionMode','Auto','PaperUnits','centimeters','PaperSize',[pos(3), pos(4)])
    set(leg3,'Position',[0.6335    0.96    0.3443    0.0504]);
    set(leg1,'Position',[0.0340    0.96    0.5959    0.0504]);
end
X = cxvec;
end